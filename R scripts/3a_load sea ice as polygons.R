...
inds = which(latitude > 50, arr.ind=TRUE)
...
for(year in 2012:2021){
  ...
  s[[year-2011]][s[[year-2011]] >= 0.15] <- 1
  c <- rasterToPolygons(s[[year-2011]], fun=function(x) x == 1, dissolve = T)#, levels = c(0.15))
...
for(year in 2012:2021){
  ...
  a[[year-2011]][a[[year-2011]] >= 0.15] <- 1
  c <- rasterToPolygons(a[[year-2011]], fun=function(x) x == 1, dissolve = T)
...
saveRDS(autumn_c, file = "data/Autumn sea ice concentration 50N 15% polygon 2012-2021.RDS")
saveRDS(spring_c, file = "data/Spring sea ice concentration 50N 15% polygon 2012-2021.RDS")
